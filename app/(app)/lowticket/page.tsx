...
export const metadata = { title: "Low Ticket — UmbraAds" };
export const dynamic = "force-dynamic";

const FREE_VISIVEL = 4;

export default async function LowTicketPage({
  searchParams,
}: {
  ...
  const categoria =
    ...
    LOWTICKET_CATEGORIAS.some((c) => c.key === searchParams.categoria)
      ? searchParams.categoria
      : LOWTICKET_CATEGORIAS[0]!.key;
    ...
    <main className="mx-auto w-full max-w-[1320px] px-8 pb-12 pt-7">
      <div className="mb-5">
        ...
        <p className="mt-2 text-[15px] text-zinc-400">
          Ofertas low ticket mineradas por categoria — Brasil, EUA, Alemanha e
          Japão.
        </p>
      </div>
      ...
      <div className="mb-6 rounded-[14px] border border-line bg-surface p-4">
        <div className="flex max-h-[132px] flex-wrap gap-2 overflow-y-auto">
          {LOWTICKET_CATEGORIAS.map((c) => {
            const ativo = c.key === categoria;
            return (
              <Link
                key={c.key}
                href={`/lowticket?categoria=${c.key}`}
                className={cn(
