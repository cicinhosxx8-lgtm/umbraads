...
/**
 * Grid do feed SaaS + "Carregar mais". Carrega por cursor em /api/saas.
 * Modo `locked` (Free): 4 reais + N cards bloqueados.
 */
export function SaasFeed({
  initialAds,
  initialCursor,
  categoria,
  locked = false,
  ...
  async function carregarMais() {
    ...
    try {
      ...
      const res = await fetch(`/api/saas?${params.toString()}`);
      ...
      {locked ? (
        <div className="mt-8 rounded-2xl border border-line bg-surface px-6 py-8 text-center">
          <div className="text-[17px] font-bold text-zinc-100">
            Você está vendo {ads.length} ofertas SaaS no plano Free 🔒
          </div>
          <p className="mx-auto mt-2 max-w-md text-sm text-zinc-400">
            Desbloqueie a mineração completa de Micro-SaaS/SaaS por categoria.
          </p>
          <Link
            href="/ajustes"
